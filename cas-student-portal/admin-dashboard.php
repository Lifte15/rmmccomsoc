...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin' && $_SESSION['department'] === 'CAS') { // Check if the role is set and it's 'Admin'
  ?>
  ...
  <head>
    ...
    <title>Admin Dashboard | CAS Student Portal </title>
  ...
  <body class="hold-transition sidebar-mini layout-fixed">
      ...
      <div class="content-wrapper">
        ...
        <section class="content">
          <div class="container-fluid">
            ...
            <div class="row">
              <?php
              $officerquery = "SELECT COUNT(*) AS count FROM user WHERE role = 'Officer' AND department='CAS'";
              ...
              $paymentforquery = "SELECT COUNT(*) AS count FROM payment_for
                 ...
                 AND payment_for.department = 'CAS'";
    ...
    // BAELS
    $BAELSProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
    ...
    $BAELSProgramresult = mysqli_query($conn, $BAELSProgramquery);

    if ($BAELSProgramresult) {
      $BAELSProgramrow = mysqli_fetch_assoc($BAELSProgramresult);
      $BAELSProgramCount = $BAELSProgramrow['count'];
    } else {
      $BAELSProgramCount = 0;
    }

    // BAPsyc
    $BAPsycProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BAPsyc' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BAPsycProgramresult = mysqli_query($conn, $BAPsycProgramquery);

    if ($BAPsycProgramresult) {
      $BAPsycProgramrow = mysqli_fetch_assoc($BAPsycProgramresult);
      $BAPsycProgramCount = $BAPsycProgramrow['count'];
    } else {
      $BAPsycProgramCount = 0;
    }

    // BACommArts
    $BACommArtsProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BACommArts' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BACommArtsProgramresult = mysqli_query($conn, $BACommArtsProgramquery);

    if ($BACommArtsProgramresult) {
      $BACommArtsProgramrow = mysqli_fetch_assoc($BACommArtsProgramresult);
      $BACommArtsProgramCount = $BACommArtsProgramrow['count'];
    } else {
      $BACommArtsProgramCount = 0;
    }

    // BSES
    $BSESProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSES' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSESProgramresult = mysqli_query($conn, $BSESProgramquery);

    if ($BSESProgramresult) {
      $BSESProgramrow = mysqli_fetch_assoc($BSESProgramresult);
      $BSESProgramCount = $BSESProgramrow['count'];
    } else {
      $BSESProgramCount = 0;
    }

    // BSMath
    $BSMathProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSMath' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSMathProgramresult = mysqli_query($conn, $BSMathProgramquery);

    if ($BSMathProgramresult) {
      $BSMathProgramrow = mysqli_fetch_assoc($BSMathProgramresult);
      $BSMathProgramCount = $BSMathProgramrow['count'];
    } else {
      $BSMathProgramCount = 0;
    }

    // BSSW
    $BSSWProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSSW' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSSWProgramresult = mysqli_query($conn, $BSSWProgramquery);

    if ($BSSWProgramresult) {
      $BSSWProgramrow = mysqli_fetch_assoc($BSSWProgramresult);
      $BSSWProgramCount = $BSSWProgramrow['count'];
    } else {
      $BSSWProgramCount = 0;
    }

    // BPA
    $BPAProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BPA' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BPAProgramresult = mysqli_query($conn, $BPAProgramquery);

    if ($BPAProgramresult) {
      $BPAProgramrow = mysqli_fetch_assoc($BPAProgramresult);
      $BPAProgramCount = $BPAProgramrow['count'];
    } else {
      $BPAProgramCount = 0;
    }

    // BPA_Dance
    $BPA_DanceProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
    ...
    if ($BPA_DanceProgramresult) {
      $BPA_DanceProgramrow = mysqli_fetch_assoc($BPA_DanceProgramresult);
      $BPA_DanceProgramCount = $BPA_DanceProgramrow['count'];
    } else {
      $BPA_DanceProgramCount = 0;
    }

    // BSBio
    $BSBioProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSBio' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSBioProgramresult = mysqli_query($conn, $BSBioProgramquery);

    if ($BSBioProgramresult) {
      $BSBioProgramrow = mysqli_fetch_assoc($BSBioProgramresult);
      $BSBioProgramCount = $BSBioProgramrow['count'];
    } else {
      $BSBioProgramCount = 0;
    }

    // BSESS_FSM
    $BSESS_FSMProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
    ...
    if ($BSESS_FSMProgramresult) {
      $BSESS_FSMProgramrow = mysqli_fetch_assoc($BSESS_FSMProgramresult);
      $BSESS_FSMProgramCount = $BSESS_FSMProgramrow['count'];
    } else {
      $BSESS_FSMProgramCount = 0;
    }
    ?>
    ...
    <script>
      $(function () {
        ...
        var BAELSProgramCount = <?php echo $BAELSProgramCount; ?>;
        var BAPsycProgramCount = <?php echo $BAPsycProgramCount; ?>;
        var BACommArtsProgramCount = <?php echo $BACommArtsProgramCount; ?>;
        var BSESProgramCount = <?php echo $BSESProgramCount; ?>;
        var BSMathProgramCount = <?php echo $BSMathProgramCount; ?>;
        var BSSWProgramCount = <?php echo $BSSWProgramCount; ?>;
        var BPAProgramCount = <?php echo $BPAProgramCount; ?>;
        var BPA_DanceProgramCount = <?php echo $BPA_DanceProgramCount; ?>;
        var BSBioProgramCount = <?php echo $BSBioProgramCount; ?>;
        var BSESS_FSMProgramCount = <?php echo $BSESS_FSMProgramCount; ?>;
        var donutData = {
          labels: [
            'BAELS',
            'BAPsyc',
          ...
          datasets: [
            {
              data: [BAELSProgramCount, BAPsycProgramCount, BACommArtsProgramCount, BSESProgramCount, BSMathProgramCount, BSSWProgramCount,
              BPAProgramCount, BPA_DanceProgramCount, BSBioProgramCount, BSESS_FSMProgramCount
              ],
