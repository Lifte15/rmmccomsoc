...
if (isset($_POST['addAnnouncement'])) {
    ...
    if (strlen($heading) > 255) {
        ...
        $sql_newAnnouncement_query = "INSERT INTO announcement(heading, content, position, posted_on, school_year, semester, department)
        VALUES(?, ?, ?, ?, ?, ?, ?)";
        $stmt_newoAnnouncement_query = mysqli_prepare($conn, $sql_newAnnouncement_query);
        mysqli_stmt_bind_param($stmt_newoAnnouncement_query, "sssssss", $heading, $content, $postedBy, $time, $school_year, $semester, $department);
