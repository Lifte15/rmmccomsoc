...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin' && $_SESSION['department'] === 'COAHS') {
  ?>
  ...
  <head>
    ...
    <title>Admin Dashboard | COAHS Student Portal </title>
  ...
  <body class="hold-transition sidebar-mini layout-fixed">
      ...
      <div class="content-wrapper">
        ...
        <section class="content">
          <div class="container-fluid">
            ...
            <div class="row">
              <?php
              $officerquery = "SELECT COUNT(*) AS count FROM user WHERE role = 'Officer' AND department='COAHS'";
              ...
              $paymentforquery = "SELECT COUNT(*) AS count FROM payment_for
                 ...
                 AND payment_for.department = 'COAHS'";
    ...
    // BSN
    $BSNProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
    ...
    $BSNProgramresult = mysqli_query($conn, $BSNProgramquery);

    if ($BSNProgramresult) {
      $BSNProgramrow = mysqli_fetch_assoc($BSNProgramresult);
      $BSNProgramCount = $BSNProgramrow['count'];
    } else {
      $BSNProgramCount = 0;
    }

    // BSP
    $BSPProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSP' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSPProgramresult = mysqli_query($conn, $BSPProgramquery);

    if ($BSPProgramresult) {
      $BSPProgramrow = mysqli_fetch_assoc($BSPProgramresult);
      $BSPProgramCount = $BSPProgramrow['count'];
    } else {
      $BSPProgramCount = 0;
    }

    // BSM
    $BSMProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                     FROM user u
                     INNER JOIN enrolled e ON u.account_number = e.account_number 
                     WHERE u.role = 'Student' 
                     AND u.program = 'BSM' 
                     AND e.school_year = '$schoolYear' 
                     AND e.semester = '$semester'";
    $BSMProgramresult = mysqli_query($conn, $BSMProgramquery);

    if ($BSMProgramresult) {
      $BSMProgramrow = mysqli_fetch_assoc($BSMProgramresult);
      $BSMProgramCount = $BSMProgramrow['count'];
    } else {
      $BSMProgramCount = 0;
    }
    ?>
    ...
    <script>
      $(function () {
        ...
        var BSNProgramCount = <?php echo $BSNProgramCount; ?>;
        var BSPProgramCount = <?php echo $BSPProgramCount; ?>;
        var BSMProgramCount = <?php echo $BSMProgramCount; ?>;
        var donutData = {
          labels: [
            'BSN',
            'BSP',
          ...
          datasets: [
            {
              data: [BSNProgramCount, BSPProgramCount, BSMProgramCount],
