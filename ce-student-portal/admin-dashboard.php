...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin' && $_SESSION['department'] === 'CE') {
  ...
  <body class="hold-transition sidebar-mini layout-fixed">
    ...
    // BSCE
    $BSCEProgramquery = "SELECT COUNT(DISTINCT u.account_number) AS count 
                 FROM user u
                 ...
                 AND u.department='CE'";
    $BSCEProgramresult = mysqli_query($conn, $BSCEProgramquery);

    if ($BSCEProgramresult) {
      $BSCEProgramrow = mysqli_fetch_assoc($BSCEProgramresult);
      $BSCEProgramCount = $BSCEProgramrow['count'];
    } else {
      $BSCEProgramCount = 0;
    }
    ?>
    ...
    <script>
      $(function () {
        ...
        var BSCEProgramCount = <?php echo $BSCEProgramCount; ?>;
        var donutData = {
          labels: [
            'BSCE',
          ],
